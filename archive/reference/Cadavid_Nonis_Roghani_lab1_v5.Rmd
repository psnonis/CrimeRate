...
* Economic status:
    ...
    + `binnedInc`: County-level income distributed in bins
...
```{r collapse = TRUE, comment=NA}
Cancer <- separate(Cancer, col = Geography, into = c("County","State"), sep = ", ",
                   remove = FALSE)
```
...
```{r collapse = TRUE, comment=NA}
# correcting median ages over 100 years which were provided in months
Cancer$MedianAge[Cancer$MedianAge>100] <- Cancer$MedianAge[Cancer$MedianAge>100] / 12 
```
...
```{r collapse = TRUE, comment=NA}
# correcting average household sizes below 1 which were divided by 100
Cancer$AvgHouseholdSize[Cancer$AvgHouseholdSize<1] <- Cancer$AvgHouseholdSize[Cancer$AvgHouseholdSize<1] * 100 
```
...
```{r eval = FALSE}
# mean incidence across the u.s.
incidenceMean <- mean(Cancer$avgAnnCount[Cancer$avgAnnCount!=1962.667684] / 
                        Cancer$popEst2015[Cancer$avgAnnCount!=1962.667684] * 100000)
# replacing anomalous values with imputation
Cancer$avgAnnCount[Cancer$avgAnnCount==1962.667684] <- 
  incidenceMean * Cancer$popEst2015[Cancer$avgAnnCount==1962.667684] / 100000
# new cancer cases per 100,000 persons per year
Cancer$incidenceRate <- Cancer$avgAnnCount / Cancer$popEst2015 * 100000 
```

#### Choice 2 - Final Choice
```{r}
# removing just the anomalous values
...
nodes.incidenceRate <- nodes[nodes$id %in% c(as.vector(links.incidenceRate$to),
                                                       'incidenceRate'),]
...
Throughout our analysis, we identified factors that have a relatively close relationship with the variable of interest, cancer death rate, and sometimes with cancer incidence rate, which is also an important parameter to predict and understand cancer trends in a community. Some of the insights that we discovered are clearly actionable, based on which effective social/health policy could be developed and implemented to improve cancer incidence and mortality. Some others (e.g. age) can not be easily controlled or changed in communities, at least not in short term. But still such variable could indicate a higher or lower mortality and could be used for prediction.

### Actionable insights
Education: Education of adults older than 24 years old, specifically two groups. The percentage of people with up to high school and the percentage with up to bachelors degree. However, the confounding effect that age may have on the relationship between these variables and death rate needs to be taken into account for future analysis.
- Unemployment: Unemployment for people over 15 shows a significant negative correlation to Cancer death rate. This may also be connected to quality of insurance for the unemployed people. However, this is one possible area where we could affect the Cancer mortality rate by providing better job opportunities.

- Poverty: Death rates amongst counties with large percentage of the population in poverty showed a strong positive correlation. Again, this is another area were we can increase the survival rate of Cancer patients by addressing their economic status.

- Public insurance coverage: There was a positive correlation between public insurance coverage and the Cancer death rate. This seems odd at the first glance, as we would expect health insurance to be negatively correlated to the death rate. This may potentially indicate some quality issues with publicly administered health insurance plans. Furthermore, there may be other confounding relationships as possibly the population with larger percentage of public health insurance  may be from older and disadvantaged communities.

- A significant number of key variables do not have a normal distribution. Since normality of the variables used is a key assumption in many statistical models, in future analysis that try to determine these relationships in a rigourous way, it is necessary to further test for normality or apply transformations to make some of the variables closer to a normal distribution.

- We found that there is a group of variables that are closely linked to each other and  to our variable of interest, specifically, employment, education, state, household size and marital status. These relationships need to be taken into account in the activities that follow up the exploratory data analysis, to make sure that the models that capture the key relationships can isolate these specific relationships from confounding effects from third variables, especially if the objective of these models is to determine causal relationships.

## Recommendation for intervention

Below we have provided our high-level recommendation for effective interventions to control some of the factors we described above. Some of these interventions have short-term effect and some others have long-term benefits, or both. This is important to take into consideration by county-level officials who should make decisions and choices based on the severity and urgency of the situation in the community as well as available resources. As such, the cost/benefit and health outcome of such interventions should be evaluated to inform decisions.

- Education: communities should increase investment in education from early stages and improve the quality, since based on our analysis, we believe such efforts will result in reduction in risk of cancer and mortality. 
- Job creation: improvement and expansion of programs to train job-seekers and teach marketable skills to individuals in a community will result in higher income, which in turn will improve access to quality cancer care.
- Health insurance: as we found out in our analysis, a higher percentage of people with public health insurance has correlation with higher cancer incidence rate and mortality. Fortunately, the Affordable Care Act, with emphasis on value-based care, has proven to be effective in increasing care quality. But we should closely monitor the progress and outcomes of this major policy shift, and specifically in this case, measure its impact on cancer mortality.

## Future data collection and analysis

The dataset that was provided to us could be improved by better collection and recording of data, to minimize the number of errors and missing values, and the level of uncertainty. Also, there are other key variables that should be included in such cancer data, which would enable a more thorough analysis. Some data that are readily available, such as smoking and obesity statistics, would help to potentially identify other factors that could be modified through various interventions. There are also other factors that are important for this type of analysis, and therefore, in future data collections; such factors should be considered (e.g. health habits, diet, physical activity). 

Finally, our focus throughout this project was to identify variables with potential relationship with cancer incidence and mortality. Based on the outcome of this analysis, a more thorough statistical analysis (e.g. regression modeling) should be conducted to better understand the relationships between key variables. 
