...
```
There is an extreme outlier on the left tail of the distribution, but we decided to keep that as we don’t have a strong reason to believe it is an error or irrelevant to our model. .
...
- Probability of Arrest: Defined as the ratio of offenses to arrests. Using this variable, we would like to assess the hypothesis that more stringent arrest protocols and improvements in crime detection be associated with o lower crime rate.
- Probability of Conviction: Defined as the ratio of convictions to arrests. If we could reject the null hypothesis regarding the association of higher convictions/arrests with , the relationships explained by our model could be the basis of potentially actionable plan for the political campaign. For instance, upon further analysis and evaluation, it be concluded that stricter sentencing guidelines, and allocation of more resources to collect more effective evidence are effective actions in reducing crime rate.
...
- Average Prison Sentence: Defined as the prison sentence in days. As explained, depending on certain conditions and further analysis, an actionable plan could be proposed to include decisive measures such as, longer average sentence times. Also, avg. sentence values across all counties seem very low (mean = 9.689, SD = 2.8). It is not clear how average is calculated. If this is calculated as Avg. Sentence Days = Sum of all sentence terms/Total Number of Prisoners, this is an extremely low value. whether this is sample that includes cer. That said, in our analysis are performing a log transformation on this variable as explained in next sections, which will enable us to address the changes in this variable as percentage, regardless of the variable unit..interpretable results a log transformation is performed, so its effect could could be more easily reasoned in terms of a percent variation.

Out decision regarding log transformation is based on the comparison of the two charts below. The histogram of `crmrate` shows a positive skew that is improved post-transformation. The distribution of log-transformed variable looks closer to normal. Since this variable can not take any negative value, the log transformation is justified.

```{r avgsen_plot, fig.height = 3}
p1 <- pHist(    crime$avgsen , breaks = 40, label = "Average Sentence"      )
p2 <- pHist(log(crime$avgsen), breaks = 40, label = "Average Sentence (log)")
...
Looking at the summary statistics of the four variables, we do not see any alarming issues. Furthermore, the histograms show a fairly normal distribution for explanatory variables, other than `avgsen`. Although, there is one extreme outlier in `prbarr` and one in `prbarr` (both from the same observation, county 115), we decided to keep them in our dataset because there is no reason to believe it’s an erroneous value. That said,, the influence of these extreme points will be evaluated during the diagnostic analysis of the regression model. 
...
1. <b>Linear in Parameters</b><br>
    ...
    Our model is linear in parameters as shown above, in the specification.

2. <b>Random (i.i.d) Sampling</b><br>
    
    We don’t have sufficient insight into how the data have been collected. For example, we don’t know if the probability of arrest is calculated by dividing the number of all arrests by the number all crimes across counties in 1987, or from a sample. But since the data are collected by key government agencies and used for analysis by reputable researchers, we assume random sampling in terms of sampling and data collection methodology. That said, we don’t have data from some of counties and we are not sure how including additional data from those counties would affect our analysis (also as explained above the counties with missing data look geographically clustered. As a result, the insights from our regression modelling might not be statistically accurate or applicable for the entire North Carolina, unless we have full insight into the missing data.
    
    An additional concern that we have with the sampling is that the data we have is only for year 1987. This specific year is know for the biggest crash in the stock market in one single day on October 19. If this event created specific conditions like an abnormal number of crime offenses (i.e. due to poverty, depression, anger, etc.) our sample is biased.

3. <b>No Perfect Multicollinearity</b><br>
    
    No evidence of perfect collinearity among the model’s explanatory variables we found, as shown below in the correlation matrix. Furthermore, R would warn users if such collinearity exists, which did not occur throughout the analysis.

4. <b>Zero Conditional Mean</b><br>
    
    ```{r m1_residuals, fig.height = 3}
    pHist(m1$residuals, breaks = 20, label = "Residuals")
    ```
    
    The histogram of residuals show a fairly normal distribution where the bulk of the data points are (except for a few spikes that don’t look totally abnormal). However, we could see the extreme outlier on the left that creates some sort of negative skew in the distribution. By observing a fairly normal distribution in the histogram of residuals and also because we have 90 observation, we can invoke the central limit theorem and therefore, assume the zero-conditional mean assumption is met.

5. <b>Homoscedasticity</b><br>
    
    Despite the fact that points on chart seem to spread out as we move to the right, there is not strong sign of heteroskedasticity, since we have very few points on the left side. However, the residual from observation row 51 (county 115), has a high influence on our model, with a Cook’s distance between 0.5 and 1. In order to have more certainty on meeting the homoscedasticity assumption, the Breusch-Pagan test was used. As the result p-value <b>`r bptest(m1)$p.value`</b> was greater than 0.05, the null hypothesis (absence of heteroskedasticity) can not be rejected. In other words, there is not enough evidence to claim our model has heteroskedasticity, and therefore, it is safe to assume homoscedasticity.

6. <b>Normality</b><br>
    
    When we analyze the Q-Q Plot, most points are on, or fairly close to the diagonal line,which might hint at normal distribution of residuals. However, we see a little bit of deviation towards the two ends of the line. Thus, we will use the Shapiro-Wilk test. the result p-value <b>`r shapiro.test(m1$residuals)$p.value`</b> is greater than 0.05 the null hypothesis (that residuals are drawn from a population with normal distribution) can not be rejected. In other words, there is not enough evidence to assume the residuals in our model are not normally distributed. Because of that, we can assume normality.
...
- $\beta_0$ : `intercept`  is `r coef(m1)[1]`, which can not be interpreted in a meaningful way without considering the other coefficients
- $\beta_1$ : `prbarr`     is `r coef(m1)[2]`, a 1% increase in probability of arrest,     crime rate will go down by approximately `r coef(m1)[2]/100`%
- $\beta_2$ : `prbconv`    is `r coef(m1)[3]`, a 1% increase in probability of conviction, crime rate will go down by approximately `r coef(m1)[3]/100`%
- $\beta_3$ : `prbpris`    is `r coef(m1)[4]`, a 1% increase in probability of prison,     crime rate will go up   by approximately `r coef(m1)[4]/100`%
- $\beta_4$ : `avgsen.log` is `r coef(m1)[5]`, a 1% increase in the average sentence,      crime rate will go up   by approximately `r coef(m1)[5]/100`%

It is worth noting that the coefficients for `prbarr` and `prbconv` amplify the effect of increasing `prbarr` or `prbconv`. This is not true for the coefficients for `prbpris` and `avgsen.log`. Another point worth noting is that, while the sign of coefficients of `prbarr` and `prbconv` are negative,behave as we expected, the positive coefficient signs forof `prbpris` and `avgsen.log` are contradictory to our hypothesis.do not. An increase in those variables is associated with an increase in crime rates, the opposite of the effect we expected.
...
<TODO:Placeholder for robust error>
<TODO:Placeholder for practical significance>


<b>Goodness of Fit</b>

* The R-squared value for the base model is 0.467, which means approximately  47% of the variation in crime rate could be explained by our model.
* The adjusted R-square value for the base model is 0.442, and because it penalizes adding more variables, it is a better comparison indicator for our upcoming models, since the simple R-square can only go up when adding more variables.
* The Akaike Information Criterion value for the base model is 102, which we will compare with those of the next models to evaluate goodness of fit.

## Research Interpretation

As evidenced by the coefficients of our base model, we can state that increases in probability of arrest and probability of conviction are associated with reduction in crime rate. In other words, a policy focused on further evaluation and potential implementation of more stringent arrest protocols and stricter criminal sentencing could be proposed to the political campaign.
The probability of prison has a positive coefficient in our base model; one might conclude that a higher prison to conviction ratio is associated with higher crime rate. However, we don’t believe that is the right interpretation  We think that this might be due to the fact that the prison to conviction ratio is already high in areas where crime rate is high. For a more effective assessment of such a relationship, we need to have data to observe the trends in crime rate before and after a policy change aimed at higher prison to conviction ratio.  An additional possible explanation is that incarceration does not deter crime as it exposes the prisoners to an environment which could reinforce  their criminal behaviour when they finish their sentence. 
The log of average sentence has a positive coefficient in our base model. Intuitively, we don’t expect  higher sentence times to drive up crime rate. We understand this might be due to the nature of our dataset, focused on 1987 only. Maybe areas with higher crime rates naturally issued more sentences and higher sentence times, as a measure to control  hight crime rate, however, the effect is yet to be noticed in the upcoming years.
...
3. No Perfect Multicollinearity</b><br>
...
For the third model, we define a specification with almost all variables in the dataset. In particular, we are interested in the inclusion of demographics and socio-economic variables that are usually related to crime rate and that would  extract probable omitted variables from the error term. We are including the following:
    
* <b>Density</b>, defined as 100 persons per square mile. Using this variable, we would like to assess the hypothesis (based on background knowledge) that  higher density is associated to higher crime rates.  We also note that this variable has a confounding effect with prbarr and prbconv.  A criminal in a city would likely have a lower probability of being recognized and therefore a lower probability of being arrested/convicted. We will use the log transformation of density in the specification with two goals: improve the "normality" of the distribution and ease of interpretation. Using this variable, we would like to assess the hypothesis that more stringent arrest protocols and improvements in crime detection would lead to lower crime rate.

* <b>Tax Revenue per Capita</b>, would likely be a determinant factor for crime rate.  A county with higher taxpc, would probably invest more in their law enforcement capabilities, increasing the probabilities of getting caught, convicted and punished.  

* <b>Percent Minority</b>, a common narrative is that minorities are more likely to be arrested for some crimes, we want to measure the effect of minorities percent on the crime rate. We do understand that this variable is from seven years earlier and may not correctly represent the present minority population.

* <b>Percent Young Males</b>, according to our background knowledge, men commit more crimes than women and younger men commit more crimes than older men.  In the context of our research question, being young makes you unaware of the probabilities of punishment? Do young people think they are invincible, are they more reckless?

* <b>Weekly Wages [Private]</b>, for different industries (wcon + wtuc + wtrd + wfir + wser + wmfg), higher wages would would likely be associated with a lower crime rate as potential offenders have an incentive to obtain money through legal means (however not all crimes have a pecuniary goal). 

* <b>Weekly Wages [Public]</b>, for government employees (wfed + wsta + wloc), in the context of our research question, higher wages would be likely be associated with better salaries for local police. Under the assumption that better salaries mean more motivated police and in addition is accompanied by better equipment/technology, this would increase the perception of being caught/convicted.  Note that we are not including weekly wages for state and federal employees as we would not know how to interpret them by count. For all wages variables we take a log transformation, in order to ease the practical significance interpretation.

With this third specification, we do not expect to obtain the most <b>parsimonious model</b> that describes our population, and expect it to possibly be an <b>overfitting model</b>.
...
We will evaluate the statistical significant of the model and use it to test the robustness of our extended model.
...
3. <b>No Perfect Multicollinearity</b><br>
    ...
    R would warn users if such collinearity exists, which did not happen throughout our analysis. To further assess the absence of presence of  multicollinearity c, we applied the vif test to our model, the results show that none of the regressors is closely predicted from the other regressors, all values are below 2.7.
...
5. <b>Homoscedasticity</b><br>
    ...
    The residuals vs. fitted values plot does not seem to indicate heteroskedasticity, the band seems to have even thickness, with the exception of the extremes where we have very few observations.  However, it is not a perfect band, the observations in the center seem to have a greater variance, therefore we examined the scale-location plot. This plot does not completely allow us to discard heteroskedasticity. We ran the Breusch-Pagan test to confirm the presence of heteroskedasticity. The null-hypothesis for this test is absence of heteroskedasticity.  The p-value we obtained is  <b>`r bptest(m3)$p.value`</b>, meaning that we can not reject the null-hypothesis.  However, as it is close to 0.05, we need to proceed with robust standard errors. 
...
6. <b>Normality</b><br>
    ...
    The plot shows a fairly normal distribution, with slight leftward skew. We also examined the Normal Q-Q plot.  The plot shows that the majority of observations are aligned with the diagonal line: however, we can also see deviations from the diagonal line on the two extremes of the chart. We also ran  the formal Shapiro-Wilk test of normality. The p-value of <b>`r shapiro.test(m2$residuals)$p.value`</b> is smaller than 0.05, meaning that we can reject the null hypothesis (sampling distribution of our coefficients is normal). However, our sample size is 90, so we will rely on the Central Limit Theorem to assume normality of errors.
...
- $\beta_3$  : `avgsen.log`    is `r coef(m3)[ 5]`, every 1% increase in average prison sentence   is associated with a `r coef(m3)[ 5]`% increase in crime rate
- $\beta_4$  : `polpc.log`     is `r coef(m3)[ 6]`, every 1% increase in police per capita         is associated with a `r coef(m3)[ 6]`% increase in crime rate
- $\beta_7$  : `density`       is `r coef(m3)[ 8]`, every 1% increase in density                   is associated with a `r coef(m3)[ 8]`% increase in crime rate
...
The goal of the political campaign is to reduce crime rate. Our analysis is not a causal model, because we can’t measure (or even identify) all variables that might cause crime rate and these omitted variables might affect the results or our conclusions. Furthermore, we do not have data from subsequent years after implementing a crime policy change to causally confirm the effect.
