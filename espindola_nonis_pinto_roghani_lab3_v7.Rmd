...
m1.fs <- round(waldtest(m1)$F[2], 2)
m2.fs <- round(waldtest(m2)$F[2], 2)
m3.fs <- round(waldtest(m3)$F[2], 2)

m1.fr <- round(waldtest(m1, vcov = vcovHC(m1))$F[2], 2)
m2.fr <- round(waldtest(m2, vcov = vcovHC(m2))$F[2], 2)
m3.fr <- round(waldtest(m3, vcov = vcovHC(m3))$F[2], 2)
...
stargazer(m1, m2, m3, type = "html", title = "Linear Models of Crime Rate",
          ...
          add.lines = list(c("F Statistic - Standard", m1.fs, m2.fs, m3.fs),
                           c("F Statistic - HC Robust", m1.fr, m2.fr, m3.fr)))
```
