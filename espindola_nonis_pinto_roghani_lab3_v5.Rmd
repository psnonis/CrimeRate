...
The study of our research question requires extended  data analysis to choose the correct variables that identify the true relationship we are trying to model. The section below details the process of defining our base model. Our aim is to find the correct variables that can answers our already defined research question.
...
The crime rate histogram shows a significant positive skew in its distribution. Therefore, a log transformation of the variable allows the outlying data points to be pulled in closer to the bulk of the observations improving its normality. Furthermore, this improves the interpretability of the model by allowing us to explain effects from the regressors in terms of percent change on the regressand.

```{r crmrte_trans}
crime$crmrte.log <- log(crime$crmrte) # log transformation
```
There is an extreme outlier on the left tail of the distribution, but we decided to keep that as we don’t have a strong reason for removing it.
...
- Average Prison Sentence: Defined as the prison sentence in days. Issuing longer sentence times, might affect the perception of the severity of punishment, and inhibit criminal activity. Moreover, this variable can provide an actionable mechanism to impact our desired outcome, as crime policy and sentencing guidelines could be updated through legislations.

The sample of  average sentence times in days in the dataset is not completely clear; one would assume prison sentences to be much longer. Possibly, this average includes convictions when no prison time was handed out. Regardless, for Average Prison Sentence to provide more interpretable results a log transformation is performed, so its effect could could be more easily reasoned in terms of a percent variation.
...
Looking at the summary statistics of the four variables, we do not see any alarming issues. Furthermore, the histograms show a fairly normal distribution for all explanatory variables. Although, there is one extreme outlier in `prbarr` and one in `prbarr` (both from the same observation, county 115) we decided to keep them in our dataset because no reason was found to consider them erroneous. However, the influence of these extreme points will be evaluated during the diagnostic analysis of the regression model. 
...
`avgsen.log` also points toward a more quadratic relationship with `crmrte.log`, but making such further transformation to the avgsen variable (already logarithmically transformed), will damage the interpretability of our model, so we will leave `avgsen.log` as is.
...
It is worth noting that the coefficients for `prbarr` and `prbconv` amplify the effect of increasing `prbarr` or `prbconv`. This is not true for the coefficients for `prbpris` and `avgsen.log`. Another point worth noting is that, while `prbarr` and `prbconv` behave as we expected, `prbpris` and `avgsen.log` do not. An increase in those variables is associated with an increase in crime rates, the opposite of the effect we expected.

On the statistical significance, we can only reject that the true coefficients for Probability of Arrest and Probability of Conviction are zero. There is not enough evidence however, to support that the coefficients of Probability of Prison and Log(Average Sentence) are not zero.
...
* The R-squared value for the base model is 0.467, which means around 47% of the variation in crime rate could be explained by our model.
...
This model attempts to improve upon the base model by incorporating additional explanatory variables. We will rely on our analysis of the base model to identify weaknesses in the selected regressors and identify possible covariates that could strengthen our prediction.
...
To improve the predicting power, we decided to include two additional explanatory variables to our extended model. Below we explain the rationale for our decision.

- Police per Capita (`polpc`): We chose this covariate based on the assumption that a higher number of cops in charge would mean an unsafe environment for individuals to commit crimes, which in turn builds a safer, low-crime community. Therefore, we are expecting to observe a negative coefficient for this variable.
...
2. <b>Random (i.i.d) Sampling</b><br>
    ...
    We don’t have sufficient insight into how the data have been collected. For example, we don’t know if the probability of arrest is calculated by dividing the number of all arrests by the number all crimes across counties in 1987, or from a sample. Another issue is that we don’t have data from some of counties and we are not sure how including additional data from those counties would affect our analysis. But since the data are collected by key government agencies and used for analysis by reputable researchers, we assume random sampling.
...
6. <b>Normality</b>
    ...
    Even though the histogram of residuals does not show any significant skewness, we looked at the Normal Q-Q plot, which shows some deviations from the normal line on towards the 2 extremes of the normal line. Therefore, we ran the Shapiro-Wilk normality, which did not confirm this normality assumption (p-value < 0.05. That said, given the sample size we can assume normal sampling distribution in our model.
...
The key purpose of this model is to demonstrate the robustness of our previous results to different model specifications.  That means, to make sure that our conclusions hold under different assumptions.
