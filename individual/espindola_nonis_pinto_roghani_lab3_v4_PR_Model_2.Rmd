...
```{r density_fix}
crime$density[crime$county==173]<- crime$density[crime$county==173] * 10000v 
...
For Model 2, we decided to inlude 3 additional explanatory variables to our model. Below we explain the rationale for our decision.
...
> Tax revenue per capita(`taxpc`): Deployment of bigger police force per capita requires a county to seek additional budget; therefore, we assume that in counties with a higher number of police per capita, tax revenue per capita is alos higher. Given such intution, leaving `taxpc` in the unobserved factors would reduce the accuracy of our model. We expect to see a negative coefficient for this variable.   

- Offense mix(`mix`): A high offense mix value means a higher ratio of face-to-face offenses to other offenses. Since a face-to-face offense (e.g. assault, armed rubbery, rape) often has at least a witness (except for homecide), we assume that prabability of arrest is higher in such cases compared to other types of offenses. Also, face-to-face offenses are usually more serious that other types and could lead to higher likelihood of conviction and imprisonment as well as higher avg. sentence. Based on these assumptions, we are adding `mix` to the extended model, since it includes `prbarr`. 

Having seen positive skewness in the distributions of `polpc`, `taxpc` and `mix` in our in initial EDA and since none of these variables could include a negative value, we are considering log transformation of these variables. This is also appropriate since it mitigates the influence of positive outliers that we have identified in our data. As seen in the histograms below, since the log transformation results in distributions closer to normal for`polpc` and `mix`, we decided to use log transformations of these variables in our model. The distribution of `taxpc` did not signficantly improve post-transofrmation, so we will use the original form in our model. 
...
p3 <- pHist(    crime$mix , breaks = 20, label = "Offense Mix Ratio"      )
p4 <- pHist(log(crime$mix), breaks = 20, label = "Offense Mix Ratio (log)")
p5 <- pHist(    crime$taxpc , breaks = 20, label = "Offense Mix Ratio"      )
p6 <- pHist(log(crime$taxpc), breaks = 20, label = "Offense Mix Ratio (log)")
...
crime$avgsen.log <- log(crime$avgse) # log transformation
```

Looking at the scatterplots of dependent and the additional explanatory variables for our second model, `polpc.log` and `mix.log` do not seem to have a linear relationship with the independet variable; however, `taxpc` seems to have a linear relationship with `crmrte.log`. Based on the scatterplots, parabolic transformation might be appropriate for `polpc.log` and `mix.log`, but it would make the interpretation of coefficients very complicated and difficult. Thereofore, we will use the variables only with log transformation. 


```{r m2_correlation, warnings = FALSE, fig.align = "center", fig.width = 9}
#pScatterMatrix(crime, columns = c(26,27,28))
# scatterplotMatrix( ~ crmrte.log + polpc.log + taxpc + mix.log, data = crime)
scatterplotMatrix( ~ crmrte.log + polpc.log +  mix.log, data = crime)
```
...
### Six CLM Assumptions - M2, Summary
...
3. Multicollinearity: We did not identify any sign of perfect collinearity among our explanatory variables looking at the correelations. In addition, R would warn users if such collinearity exists, which did not happen throughout our analysis. This is confirmed by the variance inflation factor (VIF) scores below.
4. Zero Conditional Mean: The Residual vs. Fitted chart below confirms this assumption.
5. Homoskedasticity: This assumption is confirmed as explained below. 
6. Normality: Even though the histogram of residuals does not show any skewness, the Shapiro-Wilk normality test does not confirm this assumption. That said, given the sample size we can assume that the sampling distribution of coefficients is normal.

### Model Diagnostics - M2

**Multicollinearity**  

As seen below, the variance inflation factor (VIF) scores of our model coefficients are pretty, confirming the absence of collinearity. 
...
<b>Residual vs Fitted</b>
<br>
The residual vs fitted spline is mostly flat, except for some curvature with a positive slope on the right side, deviating from zero, which  might be due to the fact that we don’t have enough data points. The chart shows that the distribution of residuals around the zero line are not dependent on indpendepnt variables, and thus confirms that MLR assumption of Zero Conditional Mean.  
...
<b>Scale-Location</b>
<br>
Despite the fact that points on the Scale-Location chart seem to spread out as we move to the right (which is mostly because we have more points on the right side side), there is no strong sign of heteroskedasticity.
...
<b>Breusch-Pagan Test</b>
<br>
In order to evaluate the homoskedasticity assumption, we also used the the Breusch-Pagan test. Since the p-value <b>`r bptest(m2)$p.value`</b> is greater than 0.05, the null hypothesis (absence of heteroskedasticity) can not be rejected. In other words there is no heteroskedasticity in our model (i.e. the variance of residuals are not dpenedent on the value of the fitted outcome variable).
...
The histogram of residuals shows a fairly normal distribution where the bulk of the data points are. The small left-skew in  the histogram is caused by the extreme outlier on the left. 
...
<b>Normal Q-Q</b>
<br>
To check for the normality assumption, we also looked at the Normal Q-Q plot. Most points are on, or fairly close to the diagonal line. However, as in base model, we see some deviations towards the two extremes of the line. Thus, we will take additional steps to verify this assumption. 
...
<b>Shapiro-Wilk Test</b>
<br>
In order to further evaluate normality assumption, the Shapiro-Wilk test was used. As the result p-value <b>`r shapiro.test(m2$residuals)$p.value`</b> is smaller than 0.05 the null hypothesis (that residuals are drawn from a population with normal distribution) can be rejected. That said, given the sample size of >30 we can assume that the sampling distribution of our coefficients is normal, so assumption 6 (normality) still holds in our model.
...
<b>Residuals vs Leverage</b>
<br>
As in the base model, the residual from observation row 51 (county 115), has an influence on our model by pulling down the line. But in model 2, its Cook’s distance is much lower than what we saw in the base model (<0,5), so it is not considered high leverege point.
...
Even though an increase in avg. prison sentence is associated with lower crime rate, its coefficient is not statistically significant. This might be due to the fact that avg. prison sentece is pretty low across North Carolina (with maximuim of around 20 days), which is unusual, since most prison sentences are at least a few months. Therefore, it is worth further evaluating this matter in the state, which is going to be part of our recommendation to the political campaign.
...
* Akaike Information Criterion value is <b>`r AIC(m3)`</b> which is lower than the AIC value for base model `r AIC(m1)`, and the AIC value for our second model `r AIC(m2)`. The third model is more accurate according to this test but it is probably not the most parsimonious model.
