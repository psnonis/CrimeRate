...
map_setup() ; map_plot('crmrte', 'Crime Rate per Capita')
```
...
The county with this anomaly is Warren County. However, only the service industry wages appear to be inflated for this county compared to the weekly wage averages for all sectors across North Carolina. It is very likely this value was incorrectly multiplied by 10, and the actual value is 217.7068 instead of 2177.068. However, since we cannot confirm that with certainty, we will leave it unmodified and not discard the observation.
