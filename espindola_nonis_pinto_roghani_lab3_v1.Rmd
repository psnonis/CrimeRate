...
map_setup()
map_plot('crmrte', 'Crime Rate per Capita')
```
...
* The R-squared value for the base model is `r summary(m1)$r.squared`, which means around 45% of the variation in crime rate could be explained by our model.
* The Akaike Information Criterion value for the base model is `r AIC(m1)`, which we will compare with those of the next models to evaluate goodness of fit.
...
<br/>
In order to evaluate CLM assumption 6 `normality` the Shapiro-Wilk test was used. As the result p-value <b>`r shapiro.test(m2$residuals)$p.value`</b> is greater than 0.05 the null hypothesis (that residuals are drawn from a population with normal distribution) can not be rejected. In other words the residuals in our model are normally distributed.
...
* The R-squared value for the extended model is `r summary(m2)$r.squared`, which means around 61% of the variation in crime rate could be explained by our model. Even though this is a higher number compared to our base model, it doesn’t necessarily show a better fit because when we add variables R-squared goes up anyway. Because of this we look at the AIC value to compare the 2 models in terms of goodness of fit.
* The Akaike Information Criterion value for extended model is `r AIC(m2)`, which is lower than that of base model value `r AIC(m1)`. This confirms that model 2 is more accurate than the base model.
